...
class Index  extends React.Component {
	...
	badge (success) {
		if(success==1){
			...
			return (
				<Badge danger><Icon type="MaterialCommunityIcons" name="bookmark-outline" /></Badge>
			)
		}else{
			return (
				<Badge primary><Icon type="MaterialCommunityIcons" name="bookmark-outline" /></Badge>
			)
		}
	}
								...
								<ListItem itemDivider key={record.booking_id}>
									<Left style = {{flex: 0}}>
					             	...
					             	<Body>
										...
										<Text note>Date: {record.date_created}</Text>
										<Text note>Qty: {record.qty_sample}</Text>
				                	</Body>
						             <Right>
						                <Button>
											<Icon name="eye" />
